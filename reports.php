...
if(isset($_GET['download'])){
    ...
    fputcsv($output,['User','Event','Status','Attendance']);
    $where_clause = "";
    if(isset($_GET['event_id']) && !empty($_GET['event_id'])){
        $event_id = $conn->real_escape_string($_GET['event_id']);
        $where_clause = "WHERE registrations.event_id = '$event_id'";
    }
    $sql="SELECT users.name,events.title,registrations.status,registrations.attendance
    FROM registrations
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul>
            ...
            <li><a href="reports.php" class="active"><i class="bi bi-bar-chart-line me-2"></i> Reports</a></li>
    ...
    <div>
        ...
        <small class="text-muted">View participant attendance and download reports for events.</small>
    </div>
        ...
        <div class="search-bar">
            ...
            <div class="d-flex justify-content-end mb-4">
                <form method="GET" class="d-flex gap-2 align-items-center">
                    <select name="event_id" class="form-select" style="width:200px;">
                    ...
                    <a href="reports.php?download=1<?php echo isset($_GET['event_id']) ? '&event_id=' . $_GET['event_id'] : ''; ?>" class="btn btn-primary btn-sm" title="Download CSV">
...
if (isset($_GET['event_id'])) {
    $row = $conn->query("SELECT title FROM events WHERE id='{$_GET['event_id']}'")->fetch_assoc();
    echo ' - ' . ($row['title'] ?? 'Event not found');
} else {
    echo ' - All Events';
}
?>
        ...
        <script>
            ...
            function searchReports(searchTerm) {
                ...
                tableRows.forEach(row => {
                    ...
                    const attendance = cells[3]?.textContent.toLowerCase() || '';
                    if (user.includes(searchTerm) || event.includes(searchTerm) || status.includes(searchTerm) || attendance.includes(searchTerm)) {
